...
public class TaskTextProcessor : ITaskTextProcessor
{
    public async Task<List<TaskInfo>> ProcessTasksAsync(List<TaskLink> taskLinks, HttpClient client)
    {
    ...
    private async Task<OneOf<string, Failed>> ExtractHtmlContentAsync(string url, HttpClient client)
    {
        try
        {
